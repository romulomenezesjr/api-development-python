...
class LikedPlaylist(Base):
    ...
    playlist_id = Column(Integer, ForeignKey(
        "playlists.id", ondelete="cascade", onupdate="cascade"), primary_key=True)
...
class UserModel(Base):
    ...
    role = Column(String(255), nullable=True,
                  unique=False, server_default="user")
...
class ContentModel(Base):
    ...
    # SQLite doest have now() method
    playlist_id = Column(Integer, ForeignKey(
        "playlists.id", ondelete="cascade", onupdate="cascade"), nullable=False)
